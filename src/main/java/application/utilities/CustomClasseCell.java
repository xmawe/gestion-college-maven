        ...
        return new TableCell<Map<String, String>, String>() {
            ...
            protected void updateItem(String item, boolean empty) {
                ...
                if (empty || item == null) {
                    ...
                    String niveauIconPath = "/images/icons/3EME-xs.png"; // Default path if class name
                                                                                   // doesn't match
                    // Adjust this logic based on your class naming convention
                    if (item.contains("3eme")) {
                        niveauIconPath = "/images/icons/3EME-xs.png"; // Update path for local resource
                    } else if (item.contains("4eme")) {
                        niveauIconPath = "/images/icons/4EME-xs.png"; // Update path for local resource
                    } else if (item.contains("5eme")) {
                        niveauIconPath = "/images/icons/5EME-xs.png"; // Update path for local resource
                    } else if (item.contains("6eme")) {
                        niveauIconPath = "/images/icons/6EME-xs.png"; // Update path for local resource
                    }
